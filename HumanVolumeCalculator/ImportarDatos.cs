   ...
   public class ImportarDatos
    {
        ...
        public void CargarModelo()
        {
            if (true==dialog.ShowDialog())
            {
                ...
                GuararModelValues(dialog.FileName);
        ...
        public void GuararModelValues(string path)
        {
            if (path==null)
            {
                return;
            }
            using (this.ReaderObj = new StreamReader(path))
            {
                this.currentLineNo = 0;
                while (!this.ReaderObj.EndOfStream)
                {
                    this.currentLineNo++;
                    var line = this.ReaderObj.ReadLine();
                    if (line == null)
                    {
                        break;
                    }
                    ...
                    while (line.EndsWith("\\"))
                    {
                        var nextLine = this.ReaderObj.ReadLine();
                        while (nextLine.Length == 0)
                        {
                            nextLine = this.ReaderObj.ReadLine();
                        }
                    ...
                    if (keyword.ToLower()=="vn")
                    {
                        return;
                    }
                    if(keyword.ToLower()=="v")
                    {
                       AddVertex(values);
